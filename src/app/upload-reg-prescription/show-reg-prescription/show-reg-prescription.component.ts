...
@Component({
  selector: 'app-show-reg-prescription',
  templateUrl: './show-reg-prescription.component.html',
  styleUrls: ['./show-reg-prescription.component.css']
})
export class ShowRegPrescriptionComponent implements OnInit {

  constructor(private service: SharedService, private alertService: AlertService, private router: Router, private imageCompress: NgxImageCompressService, private route: ActivatedRoute) { }
  ...
  ngOnInit(): void {
    this.FolderID = this.localdecryption(this.route.snapshot.paramMap.get('folderid'));
  ...
  getDirectFileList() {
    ...
    if(this.directfileList.length == 0){
      this.service.getUserFileList(data).subscribe(data => {
          ...
          console.log(this.directfileList);
          //console.log(this.folderList);
        });
    }
        /* this.service.getUserFileList(data).subscribe(data => {
  ...
  localdecryption(msg) {
    ...
    var decrypted = CryptoJS.AES.decrypt(msg, encryptSecretKey).toString(CryptoJS.enc.Utf8);
